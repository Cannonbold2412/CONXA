...
**The pre-flight gate waits for sign-in (2026-09-20).** `execute_skill` no longer throws "sign in, then run the skill again" the instant a login window opens — that made callers loop (the same sentence nine times in one `recovery.log`, because each re-run short-circuited on the still-open window). After `getCachedBrowser` reports `authPending`, `server.js` awaits every opened login (`browser.js::awaitAuthPending`, built on `awaitInteractiveAuth` over the `handle.settled` promise `beginInteractiveAuth` now keeps) for up to `CONXA_AUTH_GATE_WAIT_MS` (default 45s — under the MCP SDK's 60s request timeout; Conxa Execute sets 90s and raises its own `callTool` timeout for `execute_skill`/`authenticate`), capped so at least 60s of the run's 210s execution deadline remains, then re-resolves **once** and the run continues in the same call. If sign-in is not done, the message says what is actually true (`describeAuthWait`: window still open / closed before finishing / genuine launch failure) instead of always claiming a window "just opened". `beginInteractiveAuth` now returns `reason` (`opened` / `already_open` / `reopened` / `launch_failed`) and `key`; a `launch_failed` window is never waited on, so a real Chromium failure surfaces immediately. A wait that times out only stops *this caller* waiting — the login window and its own 10-minute `LOGIN_WAIT_MS` are untouched. Covered by `runtime/test/unit/test_await_auth.js` and `runtime/test/e2e/integration_auth_gate_waits.js`.
