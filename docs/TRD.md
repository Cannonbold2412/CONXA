...
**Dedicated model (2026-09-22):** `EXECUTE_LLM_{PROVIDER,ENDPOINT,API_KEYS,MULTIMODAL_MODEL,FALLBACK_MULTIMODAL_MODEL}` (`config.py`) define one single-endpoint, **multimodal-only** deployment tagged router pool `"execute"`. When set, `llm_proxy_routes.py::_execute_route` sends every `execute_chat` turn (text or image) down the vision path to that model (`client.py::_copilot_modality` always returns `multimodal` for the task); the router never lets other pools fall onto it (`_next_available_entry`) nor lets it fall back to the shared pool. Unset = shared pool, as before.
