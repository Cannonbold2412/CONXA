...
The gate's wait is bounded (default 45s, `CONXA_AUTH_GATE_WAIT_MS`) so a slow human login never risks losing the MCP client's response; when it expires the caller is told which app is still unsigned and calls `authenticate` (bounded up to 600s, idempotent — it re-waits on the same open window) or re-runs the skill (or resumes with `resume_from` for a mid-execution failure); the next call picks up whatever session the background capture saved. There is no fixed timeout on the login window itself, and no attempt cap across separate `execute_skill` calls — each retry is a deliberate new tool call, not an in-process loop.
