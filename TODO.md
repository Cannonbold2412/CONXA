...
| P2 — High Value, Do Soon (incl. Discovered Items) | 36 | 53 | 17 |
...
## P2 — High Value, Do Soon (34 remaining / 51 total)

### BUILD-35 — Compile pipeline: legacy cleanup, dead code, silent-fallback audit (Phases 1-3 done; 4-6 open)
- **Category:** Compiler
- **Status (2026-09-14):** Phases 1-3 of 6 landed and verified (`pytest -q tests`: 1459 passed, 1 skipped, both before and after). Phases 4-6 are scoped but not started.
  ...
  - **Phases 4-6, not started — full detail was in a plan file (`there-is-too-much-stateless-zebra.md`) that no longer exists on disk once the session ended; re-derive from this note and the codebase, don't assume the old plan file is retrievable:**
    - **Phase 4 — compile-report `degraded` channel:** today `compiler/build.py::_apply_second_opinion` can have its entire LLM pass fail (`except Exception: graph, findings = WorkflowIntentGraph(), []`) and the compile report still says `status: "ok"` — indistinguishable from the pass being cleanly disabled. Add a `degraded` list to `_build_compile_report`'s output and force `status` to at least `"review_needed"` when it's non-empty; route the second-opinion failure, the `_prefetch_vision_anchors` failure (`build.py`), `llm/anchor_vision_llm.py`'s per-worker anchor failures, and `llm/llm_cache.py`'s cache-write failures through it. Separately, three cache-poisoning bugs need fixing regardless of the channel: `llm/semantic_llm.py`, `llm/vision_llm.py`, `llm/recovery_llm.py` all cache a rule-based fallback result as if it were a real model answer, so one transient provider outage poisons that cache key permanently — never cache a `source="rule_fallback"` result. `llm/vision_llm.py`'s token-overlap fallback also hardcodes `confidence=0.65` even at zero overlap — a fabricated confidence, should reflect the actual overlap. `skill_package_builder_saved_skill.py` has two that should raise rather than record: a bad `max_iterations` silently drops the entire loop step from the saved skill (around the `if max_iterations == 0: return None` path), and an unparseable wait duration invents `ms = 1000`.
    - **Phase 5 — collapse the dual build-output layout (highest risk, do last, on its own branch with extra manual verification):** `skill_package_builder_saved_skill.py::_build_workflow_from_saved_skill` writes an intermediate `output/skill_package/<slug>/skills/<workflow>/` tree (execution.json, recovery.json, input.json, sidecars `structural_fingerprint.json`/`environment.json`/`compensation_skill.json`), which `skill_package_builder_output.py::_write_skill_packs_format` then reads back off disk to produce `data/skill-packs/<company>/<group>/<skill>/` — the ONLY layout publish (`handlers/workflows.py`), the installer (`installer_builder.py`), and the runtime actually read. Six separate `except → {}` swallows exist purely because of this in-process-data-via-filesystem round trip, each silently disabling a real feature (the structural-fingerprint drift gate, the environment mismatch gate, the compensation-skill link). Target: write directly into `data/skill-packs/...` with final filenames and pass the manifest inputs in memory; delete the sidecars, the six read-back swallows, `skill_package.json` and its cross-build merge (`pack.json`'s own merge already solves the same scoped-single-workflow-build sibling problem), `_copy_skill_package_templates`/`_render_readme`/`_render_license` (verify nothing outside the installed-runtime's own `durable_context.js` discovery actually reads this `README.md`/`CLAUDE.md`/`LICENSE`/`.gitignore` before deleting — not yet independently re-verified as of this note), and `_clean_stale_artifacts`. Re-point `handlers/skill_packages.py::cmd_get_compiled_skill` (consumed by `WorkflowTests.tsx`) at the new single location. Verify with a real two-workflow build where only one workflow is (re)compiled, to prove the sibling-skill-in-`pack.json` merge still holds without `skill_package.json`'s separate merge to fall back on.
    - **Phase 6 — conxa-core storage/LLM client:** `conxa_core/storage/{workflow_store,group_store,skill_pack_store}.py` all silently drop a corrupt on-disk record from list results (a workflow that fails to parse looks exactly like a deleted workflow to the UI) — should raise or surface an explicit error record, never silently omit. `conxa_core/storage/json_store.py` ignores a failed write but still returns the path as if it succeeded, and separately skips a test-invalidation on write failure, meaning a reviewer can sign off on a workflow against stale test results from before their edit. `conxa_core/llm/client.py:~758` reshapes a non-JSON model reply into `{"text": content, "output": content}` — a well-formed dict — so every downstream `model_validate` failure gets misdiagnosed as a schema problem instead of a parse problem; return an explicit parse-error shape instead.
...
- **Business value:** a missing `bs4`, a corrupt `pack.json`, or a failed LLM pass now surfaces as a clear, immediate error instead of a customer receiving a silently worse skill package with no compile-time signal anything went wrong.
- **Technical value:** ~250 lines of dead code removed with zero behavior change (verified by an unchanged 1459/1/0 pytest result before and after); several genuine "should have failed loudly" bugs fixed, including one (the stale-README bundle-index writer) that was actively shipping wrong content to users today, not merely dead weight.
- **Dependencies:** none for Phase 4; Phase 5 should land after Phase 4 (Phase 4's `degraded` channel is a natural place to record anything Phase 5's builder rewrite can't cleanly preserve) and needs its own dedicated verification pass given how many of the original plan's own severity assumptions turned out wrong on inspection (see the Phase 1 correction note above) — budget time for re-deriving and re-verifying claims, not just executing a written plan.
- **Suggested order:** Phase 4 next (self-contained, no structural risk); Phase 5 after, alone; Phase 6 can run independently of 4/5 whenever convenient.
- **Complexity:** Phase 4 — M. Phase 5 — L, highest risk in this item. Phase 6 — M.
- **Success criteria:** `pytest -q tests` stays green throughout every phase; a compile with a failed second-opinion pass reports `status: "review_needed"` with a named `degraded` entry, never `"ok"`; a scoped single-workflow build still lists every sibling skill in `pack.json` after Phase 5; a corrupted workflow JSON produces a visible error instead of silently vanishing from a list, after Phase 6.
