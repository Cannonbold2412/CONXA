...
- **Dependencies:** none for Stage 1 (shipped). Stage 2 needs the park/runId fix above.
- **Complexity:** L (Stage 1, shipped). Stage 2: M.
- **Success criteria (Stage 1 — verified):** unit suite green (725 tests, `runtime/`), new `test/unit/test_host_browser.js` (4 tests: launch-path default, fallback-on-unreachable-endpoint-with-logging, headless never attempts the branch, no-runId never attempts the branch) green, a from-scratch integration test driving the real `browser_panel.js`/`browser_control.js` against the real `host_browser.js` over actual CDP + the actual control channel (9 checks: connect, acquire, drivable page, `storageState` cookie seeding, `newTab`, `release`/cleanup, a second full acquire/release cycle, fallback-trigger contract) all green, and the CI execution gate (`test/e2e/gate_replay.js`) passing against the local host exe with the app layer at HEAD. **Caught by that same verification, not by inspection:** `host_browser.js` was missing from `runtime/app-layer-files.json` (the single source of truth both build pipelines and `check_app_layer_files.js` read from) — the gate failed with a real `Cannot find module './host_browser'` from the packed app layer on the first run. Fixed before this item was marked done; `check_app_layer_files.js` now passes (64 modules, coverage + require-closure verified) and the gate passes.
