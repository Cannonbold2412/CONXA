...
### BUILD-4 — Wire the "Current identity" summary into Pick Element; show pruned signals in Review Selectors
- **Category:** Builder
- **Description:** Two related follow-ups from the 2026-07-10 identity-transparency work (`FIX.md`), which surfaced each selector's orthogonality class, compile-time uniqueness, source (compiler/llm/user), and a step-level compile-confidence rollup — previously computed and persisted (`identity_bundle`) but never shown in Human Edit:
  1. **Wire `StepIdentitySummary` into Phase 1.** The component (`conxa-builder/electron/renderer/src/components/retarget/StepIdentitySummary.tsx`) is built, typed, and covered by the backend fields it reads (`StepEditorDTO.identity_engines`/`.compile_confidence`), but isn't rendered from `RetargetPhasePick.tsx` yet — that file is under separate, active concurrent work, so wiring it in was deferred rather than risk clobbering in-flight changes. Once that work settles, add `<StepIdentitySummary step={step} />` to `RetargetPhasePick.tsx`'s render (same pattern as Phase 2's badges).
  2. **Review Selectors only shows offerable selectors.** `_prune_review_candidates` (`conxa_compile/editor/retarget.py`) drops non-unique / low-durability signals from the Phase 2 candidate list so nothing sub-standard can be picked as a target — correct for that job, but it means Phase 2 can show *fewer* signals than the runtime's `identity_bundle` actually carries. `StepIdentitySummary` (once wired, per #1) is deliberately unpruned and is the honest full view; consider a greyed-out "N more signals below the offer threshold" affordance in Phase 2 itself so the review screen doesn't read as a complete inventory when it isn't one.
- **Why required:** identified while building the identity-transparency panel — the intent was full parity between what the runtime trusts and what a human reviews; #1 is the one integration point that didn't land, and #2 is a pre-existing gap the same work surfaced.
- **Business value:** small but direct — a reviewer approving a step should see the same signal-quality picture the runtime resolver acts on, not a subset.
- **Technical value:** low-risk, additive; no new backend work for #1 (data already flows), and #2 is UI-only (the raw signal list is already available via `identity_engines`).
- **Dependencies:** #1 should wait until the concurrent Pick Element / re-target wizard work in `RetargetPhasePick.tsx` stabilizes, to avoid edit conflicts.
- **Suggested order:** opportunistic — next time `RetargetPhasePick.tsx` is touched for other reasons.
- **Complexity:** S for both — #1 is a one-line render addition; #2 is a small UI affordance reusing already-computed data.
- **Success criteria:** Phase 1 (Pick Element) shows the step's current identity signals with durability/orthogonality/uniqueness/source badges before any re-pick; Phase 2 (Review Selectors) makes it visually clear when signals exist beyond what's offered as pickable.
