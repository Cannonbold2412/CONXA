...
import { WorkflowSwitcher, useSelectedWorkflowId } from '@/components/WorkflowSwitcher'
...
export function CompilePage() {
  ...
  const workflowId = useSelectedWorkflowId()
  ...
  function goCompile(sessionId: string) {
    navigate(`/workflows/${encodeURIComponent(workflowId!)}/compile/${encodeURIComponent(sessionId)}`)
  }

  function goRecompile(sessionId: string) {
    navigate(`/workflows/${encodeURIComponent(workflowId!)}/compile/${encodeURIComponent(sessionId)}?mode=recompile`)
  }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {!workflowId || q.isLoading ? (
          <p className="px-6 py-6 text-sm text-zinc-500">{!workflowId ? 'Select a workflow above to compile it.' : 'Loading…'}</p>
        ) : q.isError || !q.data ? (
          <p className="px-6 py-6 text-sm text-red-400">{(q.error as Error)?.message ?? 'Not found'}</p>
        ) : !q.data.workflow.session_id ? (
          <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
            <ListChecks className="size-7 text-zinc-700" />
            <p className="text-sm font-medium text-zinc-400">Not recorded yet</p>
            <Button size="sm" variant="outline" onClick={() => navigate(`/workflows/${encodeURIComponent(workflowId)}`)}>Go to Workflow</Button>
          </div>
        ) : (
                  ...
                  <div key={wf.id} className="flex items-center gap-4 px-5 py-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-zinc-500">
                        Recorded {wf.recorded_at ? new Date(wf.recorded_at * 1000).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '—'}
                      </p>
                    </div>
