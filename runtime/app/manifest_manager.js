...
// defaults these functions used to hardcode (8s JSON / 120s download, strict
// non-200 rejection, no auth header).
function _fetchJSON(url, timeoutMs) {
  return httpClient.fetchJSON(url, { timeoutMs: timeoutMs || 8000 });
}

function _downloadBuffer(url, timeoutMs) {
  return httpClient.downloadBuffer(url, { requireOkStatus: true, timeoutMs: timeoutMs || 120000 });
}
