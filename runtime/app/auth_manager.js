...
function _getKeytar(logFn) {
  ...
  try {
    ...
    _keytar = {
      ...
      // re-derived from env.js in standalone dev mode) so dev tokens land in the
      // dev tree, never the prod ~/.conxa.
      _file: path.join(
        process.env.CONXA_DATA_DIR || require("./host_bridge").env().dataDir,
        "cache", ".keytar.json"
      ),
      _load() {
