...
This file is guidance for Codex when working in this repository.

---

## What This Is

Conxa is an AI-native automation platform. SaaS companies record browser workflows in the Build Studio (Electron + Python, Windows), compile them into structured skill packages with multi-signal element identity and self-healing recovery, publish them to the Conxa Cloud, and distribute them to end customers as `.exe` installers that run via Codex Desktop's MCP protocol. Execution is entirely local on the customer's machine — the cloud is coordination infrastructure, not an execution environment.
...
─────────────────────        ───────────              ──────────────────
Build Studio (Windows)  →    Render + Vercel      →   Runtime (Codex Desktop)
...
MCP tools exposed by `runtime/server.js`: `execute_skill`, `execute_sequence`, `list_skills`, `get_skill_inputs`, `get_execution_status`, `cancel_execution`, `get_runtime_status`, plus the PROD-5 schedule trio `create_schedule` / `list_schedules` / `delete_schedule` (local-only schedules; see `docs/TRD.md` §4.6). There is no `refresh_skills` tool. Concurrency (RT-3, 2026-08-22): several runs may be active at once (separate chats, or several `execute_skill` calls in one chat) — see `runtime/app/run_registry.js` and `docs/TRD.md` §4.5. `execute_skill`/`execute_sequence` are admitted up to `CONXA_MAX_CONCURRENT_RUNS` (default 5); `cancel_execution` takes an optional `run_id`, required once more than one run is active. Two runs touching the SAME external platform (e.g. both hitting Render) still serialize against each other via `runtime/app/host_lock.js` — runtime-internal isolation alone doesn't protect the platform's own application layer from concurrent writes; runs on different platforms stay fully parallel. Since PROD-5 (2026-08-26) that rule also holds ACROSS engine processes (chat-host-spawned vs scheduler-daemon-spawned) through heartbeat-stamped lock files (`runtime/app/file_lock.js`, active whenever server.js passes `locksDir`). The standalone scheduler daemon (`runtime/app/scheduler_daemon.js`) fires scheduled skills with no chat app open; schedules are local-only (`scheduler_store.js`, encrypted inputs), managed by MCP tools or `conxa-runtime.exe schedule … / runner …` subcommands (dispatched by `host/cli_schedule.js`).
