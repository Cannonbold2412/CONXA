...
def _release(slug: str, skill_slug: str, version: str):
    ...
    return client.post(f"/api/v1/workflows/v2/releases/{version}/release?skill_slug={skill_slug}")
...
def test_publish_alone_leaves_the_version_ready_and_never_moves_stable():
    r = _publish("wrk_local", "deploy", "1.0.0", "v1")
    assert r.status_code == 200, r.text
    assert release_channel.get_stable_version("wrk_local", "deploy") is None
    row = db_get(publish_routes.skillpack_versions_ns("wrk_local", "deploy"), "1.0.0")
    assert row["status"] == "ready"


def test_release_activates_a_ready_version():
    _publish("wrk_local", "deploy", "1.0.0", "v1")
    r = _release("wrk_local", "deploy", "1.0.0")
    assert r.status_code == 200, r.text
...
def test_release_requires_a_ready_version():
    ...
    r2 = _release("wrk_local", "deploy", "9.9.9")
...
def test_first_publish_creates_1_0_0_and_becomes_stable_once_released():
    r = _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    assert r.status_code == 200, r.text
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.0.0"
...
def test_second_publish_creates_a_new_immutable_version():
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    r = _publish_and_release("wrk_local", "deploy", "1.1.0", "v2")
    assert r.status_code == 200, r.text
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.1.0"

    versions = client.get(
        "/api/v1/workflows/v2/skill-packs/versions?skill_slug=deploy"
    ).json()["versions"]
...
def test_ready_version_can_be_republished_before_release():
    ...
    _publish("wrk_local", "deploy", "1.0.0", "v1")
    r = _publish("wrk_local", "deploy", "1.0.0", "v1-retry")
    assert r.status_code == 200, r.text
...
def test_failed_release_does_not_change_stable(monkeypatch):
    ...
    r = unraising_client.post("/api/v1/workflows/v2/releases/1.1.0/release?skill_slug=deploy")
    assert r.status_code == 500

    # Stable is untouched, and the attempted version is left "ready" (safe to retry).
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.0.0"
    row = db_get(publish_routes.skillpack_versions_ns("wrk_local", "deploy"), "1.1.0")
    assert row["status"] == "ready"

    events = release_channel.list_release_events("wrk_local", "deploy")
...
def test_successful_release_and_retry_after_failure_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(release_routes, "write_mutable_mirror_files", _boom)
    unraising_client = TestClient(app, raise_server_exceptions=False)
    failed = unraising_client.post("/api/v1/workflows/v2/releases/1.1.0/release?skill_slug=deploy")
    ...
    retried = _release("wrk_local", "deploy", "1.1.0")
...
def test_rollback_changes_stable_pointer_without_mutating_old_versions():
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    _publish_and_release("wrk_local", "deploy", "1.1.0", "v2")
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.1.0"

    before = db_get(publish_routes.skillpack_versions_ns("wrk_local", "deploy"), "1.1.0")

    r = client.post("/api/v1/workflows/v2/releases/1.0.0/rollback?skill_slug=deploy")
    assert r.status_code == 200, r.text
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.0.0"

    after = db_get(publish_routes.skillpack_versions_ns("wrk_local", "deploy"), "1.1.0")
    ...
    rolled_to = db_get(publish_routes.skillpack_versions_ns("wrk_local", "deploy"), "1.0.0")
    assert rolled_to["is_latest"] is True


def test_rollback_to_unavailable_release_is_rejected():
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    r = client.post("/api/v1/workflows/v2/releases/9.9.9/rollback?skill_slug=deploy")
...
def test_rollback_rejects_a_version_that_was_never_released():
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    _publish("wrk_local", "deploy", "1.1.0", "v2")  # ready, never released
    r = client.post("/api/v1/workflows/v2/releases/1.1.0/rollback?skill_slug=deploy")
    assert r.status_code == 400
...
def test_unauthorized_user_cannot_publish(monkeypatch):
    ...
    r = client.post(
        "/api/v1/workflows/publish",
        json={"slug": "wrk_local", "skill_slug": "deploy", "skill_pack_version": "1.0.0", "files": _files("v1")},
        headers={
...
def test_unauthorized_user_cannot_release(monkeypatch):
    ...
    monkeypatch.setattr(settings, "api_proxy_shared_secret", "proxy-secret")
    r = client.post(
        "/api/v1/workflows/v2/releases/1.0.0/release?skill_slug=deploy",
        headers={
            "x-conxa-proxy-secret": "proxy-secret",
            "x-conxa-user-id": "user_norole",
            "x-conxa-org-id": "org_norole",
    ...
    assert r.status_code == 403
    assert release_channel.get_stable_version("wrk_local", "deploy") is None  # unaffected
...
def test_unauthorized_user_cannot_rollback(monkeypatch):
    ...
    monkeypatch.setattr(settings, "api_proxy_shared_secret", "proxy-secret")
    r = client.post(
        "/api/v1/workflows/v2/releases/1.0.0/rollback?skill_slug=deploy",
        headers={
            "x-conxa-proxy-secret": "proxy-secret",
            "x-conxa-user-id": "user_norole",
            "x-conxa-org-id": "org_norole",
    ...
    assert r.status_code == 403
    assert release_channel.get_stable_version("wrk_local", "deploy") == "1.1.0"  # unaffected
...
def test_audit_event_created_for_publish_and_release():
    ...
    published_events = client.get(
        "/api/v1/workflows/v2/releases/events?skill_slug=deploy"
    ).json()["events"]
    ...
    _release("wrk_local", "deploy", "1.0.0")
...
def test_audit_event_created_for_rollback():
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1")
    _publish_and_release("wrk_local", "deploy", "1.1.0", "v2")
    client.post("/api/v1/workflows/v2/releases/1.0.0/rollback?skill_slug=deploy")
...
def test_artifact_sha256_is_validated():
    ...
    _publish("wrk_local", "deploy", "1.0.0", "v1")
    detail = client.get("/api/v1/workflows/v2/releases/1.0.0?skill_slug=deploy").json()
...
def test_version_history_returns_correct_ordering():
    _publish("wrk_local", "deploy", "1.0.0", "v1")
    _publish("wrk_local", "deploy", "1.1.0", "v2")
    _publish("wrk_local", "deploy", "1.2.0", "v3")
...
def test_diff_endpoint_matches_preview_before_publishing():
    ...
    _publish_and_release("wrk_local", "deploy", "1.0.0", "v1", group_id="grp", extra={"grp/deploy/execution.json": "[]"})

    preview = client.post(
        "/api/v1/workflows/v2/releases/preview",
        json={
            "version": "1.1.0",
            "skill_slug": "deploy",
            "group_id": "grp",
    ...
    _publish_and_release(
        "wrk_local", "deploy", "1.1.0", "v2", group_id="grp",
        extra={"grp/deploy/execution.json": '[{"type":"click","selector":"#x"}]'},
    )
    published_diff = client.get(
        "/api/v1/workflows/v2/releases/1.1.0/diff?skill_slug=deploy"
    ).json()
...
def test_runtime_desired_version_follows_release_not_publish():
    _publish("wrk_local", "deploy", "1.0.0", "v1", extra={"_default/deploy/execution.json": '["step-a"]'})
    ...
    _release("wrk_local", "deploy", "1.0.0")
    delta_after_release = client.get("/api/v1/skill-packs/wrk_local/delta?since=%7B%7D").json()
    ...
    _publish("wrk_local", "deploy", "1.1.0", "v2", extra={"_default/deploy/execution.json": '["step-b"]'})
    delta_still_old = client.get("/api/v1/skill-packs/wrk_local/delta?since=%7B%7D").json()
    deploy_still_old = next(s for s in delta_still_old["skills"] if s["name"] == "deploy")
    assert deploy_still_old["version"] == "1.0.0"

    _release("wrk_local", "deploy", "1.1.0")
    delta = client.get("/api/v1/skill-packs/wrk_local/delta?since=%7B%7D").json()
...
def test_new_skill_first_publish_needs_no_previous_version():
    ...
    _publish_and_release("wrk_local", "existing-skill", "1.0.0", "v1")
    r = _publish_and_release("wrk_local", "brand-new-skill", "1.0.0", "v1")
    assert r.status_code == 200, r.text
    assert release_channel.get_stable_version("wrk_local", "brand-new-skill") == "1.0.0"
...
def test_publishing_one_skill_never_touches_a_siblings_version_or_stable_pointer():
    ...
    _publish_and_release("wrk_local", "update-opportunity", "1.0.0", "opp-v1")
    _publish_and_release("wrk_local", "create-a-lead", "1.0.0", "lead-v1")

    # Publishing create-a-lead's v1.1.0 must not touch update-opportunity at all.
    r = _publish_and_release("wrk_local", "create-a-lead", "1.1.0", "lead-v2")
    assert r.status_code == 200, r.text

    assert release_channel.get_stable_version("wrk_local", "create-a-lead") == "1.1.0"
    assert release_channel.get_stable_version("wrk_local", "update-opportunity") == "1.0.0"

    lead_versions = client.get(
        "/api/v1/workflows/v2/skill-packs/versions?skill_slug=create-a-lead"
    ).json()["versions"]
    opp_versions = client.get(
        "/api/v1/workflows/v2/skill-packs/versions?skill_slug=update-opportunity"
    ).json()["versions"]
...
def test_rolling_back_one_skill_never_touches_a_siblings_files_or_version():
    _publish_and_release("wrk_local", "skill-a", "1.0.0", "a-v1", extra={"_default/skill-a/execution.json": '["a-step-1"]'})
    _publish_and_release("wrk_local", "skill-a", "1.1.0", "a-v2", extra={"_default/skill-a/execution.json": '["a-step-2"]'})
    _publish_and_release("wrk_local", "skill-b", "1.0.0", "b-v1", extra={"_default/skill-b/execution.json": '["b-step-1"]'})

    client.post("/api/v1/workflows/v2/releases/1.0.0/rollback?skill_slug=skill-a")

    assert release_channel.get_stable_version("wrk_local", "skill-a") == "1.0.0"
    assert release_channel.get_stable_version("wrk_local", "skill-b") == "1.0.0"  # untouched
...
def test_put_group_shows_up_empty_on_get_and_rename_updates_name():
    ...
    r = client.put(
        f"/api/v1/workflows/v2/groups/{gid}",
        json={"group_name": "Sales", "company_name": "Acme"},
    )
    assert r.status_code == 200, r.text
    assert r.json() == {"slug": "wrk_local", "group_id": gid, "group_name": "Sales"}

    listed = client.get("/api/v1/workflows/v2/groups")
...
def test_publish_attaches_to_existing_synced_group_without_duplicating():
    ...
    p = _publish("wrk_local", "create-a-lead", "1.0.0", "v1", group_id=gid)
    assert p.status_code == 200, p.text
...
def test_get_groups_still_includes_skills_never_synced_as_a_group():
    p = _publish("wrk_local", "orphan", "1.0.0", "v1", group_id="legacy-grp")
    assert p.status_code == 200, p.text
    groups = client.get("/api/v1/workflows/v2/groups").json()["groups"]
