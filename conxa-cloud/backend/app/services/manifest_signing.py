...
Lives in services/, not api/: the same Ed25519 canonical-JSON scheme signs both the
update manifest (app.api.updates_routes) and the PROD-18 governance policy
(app.services.tracking) — a service importing from api/ inverted the codebase's
usual layering, so this moved here. app.api.manifest_signer re-exports these names
