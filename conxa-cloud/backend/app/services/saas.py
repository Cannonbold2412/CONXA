...
def _clerk_org_role(user_id: str, org_id: str) -> str | None:
    ...
    Called only when org_role is absent from the JWT claims (e.g. JWT template
    doesn't include it). Requires CLERK_SECRET_KEY to be set.
    """
...
def principal_from_request(request: Request) -> Principal:
    ...
    if not raw_role and raw_org_id:
        # org_role absent from JWT (Clerk JWT template doesn't include it) — fall back to API
