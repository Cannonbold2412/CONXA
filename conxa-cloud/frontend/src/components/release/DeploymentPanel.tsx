...
/** Deployment dashboard — desired vs. installed version, machine health, and
 * rollout progress. Only ever shows counts the runtime actually reported
 * (runtime_registrations) — never a fabricated percentage. */
  ...
  return (
    <div className="rounded-lg border border-white/8 bg-white/[0.02] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500">Desired version</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
        {(['up_to_date', 'pending', 'failed', 'offline', 'unknown'] as DeploymentStatusLike[]).map((status) => (
              ...
              {machines.map((m) => (
                <tr key={m.machine_id} className="border-t border-white/5 text-zinc-400">
                  ...
                  <td className="px-2.5 py-1.5">
                    <span className="flex items-center gap-1.5" title={m.sync_error?.code}>
