...
Grounding: see `current-state.md`. The recurring theme — Conxa is *Different by architecture* and *Ahead in concept* but *Behind in execution robustness* — holds across nearly every row.
...
- **Next:** Add intent/validation/confidence/conditional capture and harden the WorkArena interaction types (`recording.md`).
...
- **Next:** Introduce a diffable IR, deterministic floors, reproducible/pinned compiles, version+rollback (`compiler.md`).
...
- **Next:** Make recovery autonomous via host MCP sampling, add **healed-selector write-back** (ephemeral local + telemetry → Cloud re-sign), confidence/cause classification, and repair validation (`05-recovery.md`).
...
- **Next:** Build a minimal actionable Tier-4 (grounder → bbox → re-derive selector → outcome-check) with scaleFactor + SoM-as-telemetry (`vision.md`).
...
- **Next:** Refactor to ServerBackend seam; add entitlement-filtered `list_skills`; add an escalation/handoff tool (`mcp.md`).
...
- **Next:** This is the most important build — `workflow-durability.md` + the fleet flywheel.
...
- **Next:** Wire RBAC, add SSO, tenant isolation, compliance export, fleet registry (`enterprise.md`).
