# Edge-Case Framework (Phase 7)

**Purpose:** for each high-impact edge case, the final decision on **Detection · Representation · Replay · Recovery · Verification**. This is the *per-case playbook*; it operationalizes the *per-family handbook* in [`../05-reliability/framework.md`](../05-reliability/framework.md) into concrete cases the implementation can follow directly. Every row is zero-token unless marked `H`/`V`/`U`. EC-/RP- IDs cross-reference [`../05-reliability/inventory.md`](../05-reliability/inventory.md) / [`../05-reliability/recovery-patterns.md`](../05-reliability/recovery-patterns.md).
...
- Covered in `03-replay-algorithm.md` §5: fill→wait-async-options→select-exact (typeahead); open→wait→click-by-text (custom dropdown). Verification: the committed value equals the intended value (not merely highlighted).
