# Selector Architecture (Phase 3) — Production Standard
...
The floor guarantees identity even if the LLM is unavailable, and it cuts compile cost: the LLM is invoked **only when the floor leaves residual uncertainty** (`compiler.md` §5), driving 4–5 calls/step toward <1 amortized (the scalability win, §11).
...
**3.4 Orthogonality guarantee (≥2, by construction).** Compile **fails** if a non-marker step cannot produce ≥2 signals of *different `orthogonality_class`* (semantic-aria / test-contract / visible-text / spatial-anchor / structural). This makes "one DOM change cannot break all signals" a structural invariant, not a hope (`compiler.md` §3). v1 asserted orthogonality; this *enforces* it at compile.
...
Resolution is a single **pure, side-effect-free, zero-LLM function** — the keystone of maintainability and testability (`runtime.md` §1, the `Resolver` module):
...
**6.2 Scoring & tie-break — `compute_stable_hash`, not hand-waving.** v1 tie-broke by vague "fingerprint+anchors." Production uses browser-use's **dynamic-class-stripped stable identity hash** (`views.py:828-887`, `139-184`): a hash of the parent-branch tag path + sorted static attributes + AX name, with focus/hover/active/animation classes stripped for cross-session stability. The recorded target's `stable_hash` ships in the bundle; at resolve, multi-match is broken by **which candidate's live stable_hash matches the recorded one** — a principled, deterministic tie-break that also detects "resolved a different identical-looking node." `score()` additionally rewards role/name/testid agreement and penalizes `guid_like` attributes (`scoreCandidate`, `runtime.md` §2), versioned with the compiler so compile and replay agree.
...
- **Miss → cascade.** A MISS/AMBIGUOUS result enters the zero-token cascade (next orthogonal signal → a11y role+name re-probe → relational/anchor re-find → frame/shadow re-resolution → scroll-until-found), all reusing the same pure matcher (`05-recovery.md` Layers 1–2).
...
- **Write-back, never local mutation.** A verified above-Tier-1 resolution emits a `repair_event` (recorded signal → healed signal, post-condition result, app-version fingerprint), used **ephemerally for the run** and shipped to Cloud as a **drift signal surfaced to the conxa-cloud admin** (insight #11). A durable fix is never automatic: it requires the admin to review the surfaced drift and **manually publish** a re-signed version. The signed local bundle is never silently rewritten — the discipline Stagehand's mutable cache cannot claim.
...
1. **Compile-time ranking becomes site-specific and self-correcting** — the durability order is *learned*, not asserted; the priors inform the **next compile and the admin's review**, never an automatic rewrite of a fielded pack. (Ember GUID-ids get demoted; a strong testid contract gets promoted.)
2. **Drift is detected fleet-wide on first occurrence** — when one runtime's primary signal degrades on site X, the drift is **surfaced to the conxa-cloud admin** on first detection; the admin reviews and **manually publishes** a re-signed bundle, which then delta-syncs to all customers (the flywheel, insight #1). Detection is automatic and fleet-wide; **publishing is always admin-approved, never automatic.**
...
Memory is also *local*: the runtime caches the last-known-good resolution per step (Stagehand cache-replay shape, `ActCache.ts`) to short-circuit resolution on the warm path, while the durable fix flows through admin review and a **manually published** re-signed version.
...
- **Supply-chain integrity:** the IdentityBundle is signed in the pack (Phase 8); the matcher trusts the signed signals and re-derives everything else from the (untrusted) live DOM. A healed signal re-enters the fleet only via an **admin-reviewed, manually published** re-signed version — never local mutation and never automatic push.
...
**Scalability.**
- *Compile cost:* deterministic-first gating (§3.2) invokes the LLM only on residual uncertainty, cutting 4–5 calls/step toward <1 amortized (`compiler.md` §5) — the dominant compile-cost lever.
...
- **Stagehand:** grounds lazily at runtime (cost + non-determinism) and *mutates a local cache*; Conxa grounds once at compile, ships a signed deterministic bundle, and heals via an **admin-reviewed, manually published** re-sign — keeping signed determinism Stagehand's mutable cache cannot claim.
