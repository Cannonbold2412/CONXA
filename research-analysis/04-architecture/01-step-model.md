...
**Purpose:** the exact, final schema for a single compiled workflow step. This is the data contract between the compiler and the deterministic runtime. Every field exists because a specific failure family (`inventory.md`) or insight (`master-insights.md`) demands it; anything that earns no reliability, verification, recovery, or audit value is removed.
