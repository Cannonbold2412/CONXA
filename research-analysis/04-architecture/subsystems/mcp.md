# MCP Subsystem — Architecture Design (24-Month Target)
...
**Seam discipline:** how a *step* runs is owned by `runtime.md`; how *recovery* heals is owned by `05-recovery.md` (Stage 7 produces the handoff this doc renders). This document owns only the **protocol boundary** and hands off at two clean seams: `backend.callTool → StepEngine` and `Recovery.Stage7 → handoff response`.
