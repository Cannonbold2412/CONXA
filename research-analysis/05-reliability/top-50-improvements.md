...
**ROI** = (Reliability × Enterprise) discounted by Complexity×Risk×Difficulty. **All but 4 are zero-token (`Z`)** — `H`/`V`/`U` marked. Cross-refs: EC-xx (`inventory.md`), RP-xx (`recovery-patterns.md`), framework families (`framework.md`).
