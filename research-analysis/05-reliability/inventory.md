...
**Lens:** Conxa is deterministic (record→compile→replay; zero-LLM hot path; recovery at the edges). For each edge case the question that matters is *"can this be detected and resolved deterministically, and if not, how is it contained?"* — answered fully in `framework.md`.
