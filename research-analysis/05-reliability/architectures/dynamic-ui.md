...
Keep re-resolving every action (already correct). **Add the actionability stack** (attached→visible→**stable(RAF)**→enabled) before acting, so mid-render motion (EC-09/11) is absorbed instead of mis-clicked. This is the same gate `runtime.md` specifies — it pays off most here.
