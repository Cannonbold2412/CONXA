# Host-Vision / Computer-Use (CUA) Paradigm — Edge-Case Analysis (Phase 2)

> **This analyzes a paradigm, not a repo (historically codenamed "Fable" in this corpus).** There is no `Fable` repository in the research corpus.
...
> (`05-recovery.md`, `vision.md`). UI-TARS is the closest concrete instance
> in the corpus and is cross-referenced where useful — but the UI-TARS-specific analysis lives in
> `cross-repo-comparison.md §6` and is **not duplicated here**.
...
- **Non-deterministic** — same input, different trajectory; no replay guarantee (disqualifying for enterprise SLA, `paper-review.md §4`).
- **Expensive** — every step pays full VLM cost; nothing transfers between runs (`cross-repo-comparison.md §6`, "inference-only cannot scale").
...
- **Brittle at DPI/scale** — coordinate output is fragile across `devicePixelRatio`, zoom, responsive breakpoints (`paper-review.md §4`, coordinate-only identity).
...
**Conxa applicability — RIGHT TOOL, narrowly, for recognition only.** This maps to `05-recovery.md`
...
**compiled conditional** (`if_present`/`try_dismiss`, `05-recovery.md §4`) — deterministic
...
**Conxa applicability — WRONG TOOL.** Conxa already has a deterministic **re-auth self-heal** (`05-recovery.md`
...
`vision.md §3d` — **`elementFromPoint` hit-testing is shadow-piercing**, so even a closed root
...
the lock. This is precisely `vision.md §1` ("vision fires only when DOM/AX grounding is
...
text-first, not pixels (`05-recovery.md` T3).
...
description and resolving it deterministically does not (`paper-review.md §1`, §4).
**Recovery.** The matched node is re-derived into a multi-signal fingerprint, scored, and — crucially — its action
is **validated by an independent post-condition** (`05-recovery.md` Stage 4). Conxa's edge over
...
(`master-insights.md` R1; shared by browser-use reflection and UI-TARS SoM, both of which record *belief*, not
*outcome*).
...
durable Family-5 lesson is **outcome verification, not vision** (`paper-review.md §1`, functional success
criteria).
...
- **It produces a coordinate, not a re-resolvable selector** — a one-shot, unauditable, DPI-fragile locator. Conxa's
  ...
  (`vision.md §3`). A coordinate is not promotable to a package fix; a selector is.
