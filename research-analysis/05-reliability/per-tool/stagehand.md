...
> Source corpus: `browserbasehq/stagehand`, `packages/core/lib/v3/`. Lens: Conxa's deterministic-default (record→compile→replay, zero-LLM hot path, LLM only as recovery escalation). References EC-IDs and the five families from `inventory.md`.
